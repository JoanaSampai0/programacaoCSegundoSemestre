...
// Struct livro agora tem um campo 'disponivel' para controle o status
struct Livro {
...
int main() {
    ...
    // Agora, em vez de arrays staticos, usamos ponteiros.
    struct Livro *biblioteca;
    struct Emprestimo *emprestimos;
    
    // Usamos calloc para o array de livros. calloc (num_elementos, tamanho_de_cada_elemento)
    // Vantagem: inicializa toda a memória com zeros. Isso significa que 'disponível' começa como 0 (não disponível).
    biblioteca = (struct Livro *)calloc(MAX_LIVROS, sizeof(struct Livro));
    
    // Usamos malloc para o array de meprestimos. malloc (tamanho_total_em_bytes)
            ...
            case 1: // CADASTRO DE LIVRO
                ...
                if (totalLivros == 0) {
                    ...
                    for (int i = 0; i < totalLivros; i++) {
                        ...
                        printf(" Nome: %s\n", biblioteca[i].nome);
                        printf(" Autor: %s\n", biblioteca[i].autor);
                       // Mostrar o status de disponibilidade
                       printf(" Disponivel: %s\n", biblioteca[i].disponivel ? "Diponivel" : "Emprestado");
                    }
                    printf("-----------------------------\n");
